...
sub_base <- function(criterio) {
  return(dados_graf %>% filter(Ano == "2018", 
                               fatores == criterio))
}

sub_bases <- purrr::map(fatores, sub_base)
names(sub_bases) <- fatores


teste <- ggplot(dados_graf %>% filter(Ano %in% c("2017", "2018")), aes(y = y, yend = yend, xend = Periodo)) +
...
plot_ly() %>%
  ...
  add_segments(data = sub_bases[["Emissões"]], 
               x = ~Periodo, xend = ~Periodo, y = ~y, yend = ~yend, name = "Emissões") %>%
  add_segments(data = sub_bases[["Resgates"]], 
               x = ~Periodo+10, xend = ~Periodo+10, y = ~y, yend = ~yend, name = "Resgates") %>%
  add_segments(data = sub_bases[["Emissões"]], 
               x = ~Periodo, xend = ~Periodo + 10, y = ~yend, yend = ~yend,
               line = list(dash = "dot", width = 1), name = "") %>%
  add_segments(data = sub_bases[["Resgates"]], 
               x = ~Periodo + 10, xend = ~prox_periodo, 
