...
gera_graf <- function(indicador) {
  ...
  graf_basico <- ggplot(base, aes(x = reorder(mes_texto, Mes), group = Ano, y = Valor)) +
    ...
    geom_text(aes(label = ifelse(Mes == 1 & Ano == ano_atu - 1, 
              ...
              family = "Source Sans Pro", size = 3, hjust = "right") +
  ...
  if (unidade == "R$ bi") {
    graf_final <- graf_basico +
      geom_text(aes(label = ifelse(Periodo == max(base_GN$Periodo), 
                               ...
                               NA),
                color = dentro_fora,
                x = Mes + 0.3), family = "Source Sans Pro", hjust = "left", size = 3.5) +
      scale_y_continuous(position = "right", labels = function(x) {format(x, big.mark = ".", decimal.mark=",", scientific = FALSE)})
  } else if (unidade == "Percentual") {
    graf_final <- graf_basico +
      geom_text(aes(label = ifelse(Periodo == max(base_GN$Periodo), 
                               percent(round(Valor,0)), 
                               NA),
                color = dentro_fora,
                x = Mes + 0.5), family = "Source Sans Pro", hjust = "left", size = 3.5) +
