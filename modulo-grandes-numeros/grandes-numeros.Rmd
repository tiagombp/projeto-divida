...
#com annotate em vez de geom_ribbon
teste_graf1 <- ggplot(base_GN %>% filter(Indicador == "Estoque DPF"), aes(x = reorder(mes_texto, Mes), group = Ano, y = Valor)) +
  annotate(geom = "rect", ymin = (base_GN %>% filter(Indicador == "Estoque DPF"))$PAFmin[1], ymax = (base_GN %>% filter(Indicador == "Estoque DPF"))$PAFmax[1], xmin = -Inf, xmax = +Inf, fill = "LightBlue", alpha = 0.5) +
  geom_line(aes(y = ifelse(Ano == ano_atu, Valor, NA), color = dentro_fora),
            size = 1) +
  geom_line(aes(y = ifelse(Ano == ano_atu-1, Valor, NA)),
            color = "grey") +
