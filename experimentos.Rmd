...
```{r}
dados_tidy_2 <- dados %>%
  mutate(Fonte_cod = str_pad(Fonte.Recursos.Código, 2, pad = "0")) %>%
  group_by(Ano = Exercicio, 
           Fonte = Tipo.de.Fonte,
           Fonte_cod,
           Mod = Modalidade.da.Dívida, 
...
gera_matriz(dados, ano, criterio, apenas_mercado) {
...
plota_b <- function(ano, criterio, apenas_mercado) {
  ...
  if (apenas_mercado) {
    dadinhos <- dados_tidy2 %>% filter(Carteira == "MERCADO")
    }
    else {
      dadinhos <- dados_tidy2
    }
